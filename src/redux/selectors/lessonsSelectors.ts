...
export const selectAllLessonLoadedFlag = (state: Store) =>
  state.lessons.allLessonsLoaded;
...
export const selectFullPriceOfLessons = (lessons: ILesson[]) =>
  lessons.reduce((cur, lesson) => cur + lesson.price, 0);
export const selectMemoFullPriceOfLessons = createSelector(
  selectLessons,
  selectFullPriceOfLessons
);

export const selectAllLessonsForStudent = (lessons: ILesson[], name: string) =>
  lessons.filter((lesson) => lesson.name === name);

export const selectAmountOfLessonsForStudent = (
  lessons: ILesson[],
  name: string
) => selectAllLessonsForStudent(lessons, name).length;
export const selectMemoAmountOfLessonsForStudent = createSelector(
  [selectLessons, (_, name) => name],
  selectAmountOfLessonsForStudent
);

export const selectFullIncomePerStudent = (
  lessons: ILesson[],
  name: string
) => {
  return selectAllLessonsForStudent(lessons, name).reduce(
    (cur, lesson) => cur + lesson.price,
    0
  );
};
...
const selectLessonsPerMonthAndYear = (
  lessons: ILesson[],
  year: string,
  month: string
) => lessons.filter((lesson) => lesson.date.includes(`${year}-${month}-`));

const selectAmountOfLessonsPerMonthAndYear = (
  lessons: ILesson[],
  year: string,
  month: string
) => selectLessonsPerMonthAndYear(lessons, year, month).length;
...
const selectFullIncomeForMonthAndYear = (
  lessons: ILesson[],
  year: string,
  month: string
) => {
  return selectLessonsPerMonthAndYear(lessons, year, month).reduce(
    (cur, lesson) => cur + lesson.price,
    0
  );
};
...
const selectLessonsPerStudentPerMonthAndYear = (
  lessons: ILesson[],
  name: string,
  year: string,
  month: string
) => {
  return selectLessonsPerMonthAndYear(lessons, year, month).filter(
    (lesson) => lesson.name === name
  );
};

const selectAmountOfLessonsPerStudentPerMonthAndYear = (
  lessons: ILesson[],
  name: string,
  year: string,
  month: string
...
export const memoizedSelectAmountOfLessonsPerStudentPerMonthAndYear =
  createSelector(
    [
      selectLessons,
      (_, name) => name,
...
const selectIncomePerStudentPerMonthAndYear = (
  lessons: ILesson[],
  name: string,
  year: string,
  month: string
) => {
  return selectLessonsPerStudentPerMonthAndYear(
    lessons,
    name,
    year,
    month
...
const selectUnpaidLessonsPerStudentPerMonthAndYear = (
  lessons: ILesson[],
  name: string,
  year: string,
  month: string
...
export const memoizedSelectUnpaidLessonsPerStudentPerMonthAndYear =
  createSelector(
    [
      selectLessons,
      (_, name) => name,
...
const selectStudentsForMonthAndYear = (
  lessons: ILesson[],
  year: string,
  month: string
) => {
  return Array.from(
    new Set(
      selectLessonsPerMonthAndYear(lessons, year, month).map(
        (lesson) => lesson.name
      )
    )
  );
...
export const memoizedSelectStudentsForMonthAndYear = createSelector(
  [selectLessons, (_, year) => year, (_, __, month) => month],
