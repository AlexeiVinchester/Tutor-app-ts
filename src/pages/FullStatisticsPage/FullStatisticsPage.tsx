...
const FullStatisticsPage = () => {
  ...
  const [commonStatistics, setCommonStatistics] =
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      try {
        const responseOfNames = await fetch(
          `http://localhost:3002/getStudentsNames`
        );
        if (!responseOfNames) {
        ...
        const responseOfCommonStatistics = await fetch(
          `http://localhost:3002/getCommonStatistics`
        );
    ...
    async (name: string) => {
      ...
      try {
        const response = await fetch(
          `http://localhost:3002/getFullStudentStatistics?name=${name}`
        );
        if (!response.ok) {
        ...
        dispatch(
          showSnackMessage(
            createSnackMessage(
              `Error while loading statistics data: ${
  ...
  const handleChangeStudentName = useCallback(
  ...
  return (
    <>
      <StatisticsPageHeader description="Here you can find statistics for full time: amount of lessons and full income. In the bottom part choose student and find income and amount of lessons for interested student!" />
